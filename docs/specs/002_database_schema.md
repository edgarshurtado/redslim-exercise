...
| `value` | decimal(12,2) | No | Sales value for the period, in DKK (Danish krone) |
...
4. `Data.value` is stored as `decimal(12, 2)` in DKK (Danish krone) (e.g. `32.40`). `Data.weighted_distribution` is stored as `decimal(5, 2)` representing a percentage (e.g. `85.85` = 85.85%), or NULL when not reported.
